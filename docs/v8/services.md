...
| RustFS | Dedicated until the external admin-client lifecycle is proven | Whole authenticated project instance; bucket creation is not yet automated | Retained-volume backup; new instance for storage-format boundary | Default; share after bucket, identity, policy, and recovery acceptance tests pass |
...
| RustFS | Stable generated root access key/secret; console disabled | Generated project-private S3 endpoint and path-style SDK defaults; no bucket value, host port, or gateway route | Engine state only; the documented single-node data volume is configured, but bucket provisioning and protocol readiness remain fail-closed | Stop/remove the disposable container, disable its credential, and retain its project volume |

...
| Dragonfly | Dedicated until Redis ACL parity is proven | Whole authenticated project instance | Scheduled snapshot plus retained-volume backup/restore | Default; share only after ACL and recovery acceptance tests pass |
...
| RustFS | Dedicated until scoped IAM and policy behavior are proven | Whole authenticated project instance with one deterministic project bucket | Retained-volume backup; new instance for storage-format boundary | Default; share after scoped identity, policy, and recovery acceptance tests pass |
...
| RabbitMQ | Stable project user/password, dedicated vhost, exact permissions, and project-scoped management access | Internal AMQP host, port, vhost, username, and password | Administrator-authenticated diagnostics plus atomic definitions publication | Delete the exact project user; retain vhost, topology, and messages |
...
| Dragonfly | Stable generated `default` password; primary-port HTTP disabled | Generated project-private Redis-compatible host, port, username, and password; no host port or gateway route | A pinned private-network client performs an authenticated `PING`; failed probes retry and successful unchanged revisions do not create an Engine-event loop | Stop/remove the disposable container, disable its credential, and retain its project volume |
| Memcached | No credential; any namespace remains an application convention | Generated project-private host and port; no host port or gateway route | A pinned private-network client requires an exact Memcached `VERSION` protocol response | Stop/remove the disposable container; no persistent data is retained |
| Garage | Stable generated S3 access key/secret plus derived RPC and admin secrets in a private generated config | Generated project-private S3 endpoint and default project bucket; no host port or gateway route | v2.3 performs single-node bootstrap, then a pinned private-network S3 client authenticates and idempotently verifies the bucket | Stop/remove the disposable container, disable its credential, and retain its project volume |
| RustFS | Stable generated root access key/secret; console disabled | Generated project-private S3 endpoint, deterministic bucket, and path-style SDK defaults; no host port or gateway route | A pinned private-network client authenticates and idempotently creates the bucket; failed jobs retry and successful unchanged revisions do not create an Engine-event loop | Stop/remove the disposable container, disable its credential, and retain its project volume |
| LocalStack | Fixed public `test` SDK identity; no generated or durable Stackctl secret | Generated project-private gateway endpoint, deterministic S3 bucket, and SDK defaults; no host port or implicit route | A pinned private-network S3 client idempotently creates the bucket; persistence is enabled and failed provisioning retries | Stop/remove the disposable container; retain its project volume |
| OpenSearch | Stable generated demo administrator password satisfying the image policy | Generated project-private HTTPS endpoint and administrator identity; no host port or implicit route | A pinned private-network HTTP client authenticates to cluster health; the dedicated demo certificate is accepted only inside the private network, and safe shared security is not claimed | Stop/remove the disposable container, disable its credential, and retain its project volume |
| Elasticsearch | Stable generated `elastic` administrator password | Generated authenticated project-private HTTP endpoint; no host port, implicit route, or per-project CA | A pinned private-network HTTP client authenticates to cluster health; safe shared security is not claimed | Stop/remove the disposable container, disable its credential, and retain its project volume |
| Meilisearch | Stable generated master key; project-specific settings remain declarative | Generated project-private HTTP endpoint and key; no host port or implicit route | A pinned private-network HTTP client authenticates to the read-only keys endpoint while discarding its sensitive response; scoped-key sharing is not claimed | Stop/remove the disposable container, disable its credential, and retain its project volume |
| Typesense | Stable generated bootstrap API key; project-specific settings remain declarative | Project-private host, port, and protocol injected into the application; no host port or implicit route | A pinned private-network HTTP client authenticates to the read-only debug endpoint; scoped-key sharing is not claimed | Stop/remove the disposable container, disable its credential, and retain its project volume |

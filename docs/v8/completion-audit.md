...
| AC-18 | Persistent resources are never pruned implicitly | Implemented | retention policy tests; orphan stop behavior; exact seven-day disposable GC tests | Explicit persistent deletion/uninstall delete-data workflow remains incomplete |
...
| AC-22 | Removing/restoring config follows retention rules | Partial | atomic orphaning, credential disablement, adoption, backup, restore, and disposable GC tests | Explicit persistent prune, complete uninstall modes, and live rename/remove/restore acceptance |

...
`cargo test --quiet`: 1,494 passed, 0 failed. `just lint`, `just build`,
...
| AC-23 | Existing v7 projects have tested migration and rollback | Partial | TOML-to-YAML semantic migration; secret-free typed v7 config/container/image/mount/route/trust/runtime inventory with exact legacy-label and volume-drift blockers; reversible PostgreSQL, MySQL/MariaDB, MongoDB, and SQL Server backup/restore/cutover/confirm/rollback tests | Persist and expose inventory, capture legacy trust and generated environment artifacts, select app/route/volume adapters, add remaining data-service operations, and prove live rollback |
...
| AC-28 | Relevant unit, integration, migration, chaos, platform, build, and lint checks pass | Partial | 1,494 local tests plus lint/build at this snapshot; Unix architecture CI definition | Required live platform, migration breadth, gateway protocol, image publication, and benchmark suites above |

...
`cargo test --quiet`: 1,430 passed, 0 failed. `just lint`, `just build`,
...
| AC-22 | Removing/restoring config follows retention rules | Partial | atomic orphaning, credential disablement, adoption, PostgreSQL, MySQL/MariaDB, and MongoDB backup, crash-replayable PostgreSQL, MySQL/MariaDB, MongoDB, and SQL Server prune, disposable GC, reversible PostgreSQL, MySQL/MariaDB, and MongoDB restore/cutover/confirm/rollback, and explicit safe uninstall-mode tests | Remaining persistent-service adapters, delete-data uninstall execution, and live rename/remove/restore acceptance |
...
| AC-28 | Relevant unit, integration, migration, chaos, platform, build, and lint checks pass | Partial | 1,430 local tests plus lint/build at this snapshot; Unix architecture CI definition | Required live platform, migration breadth, gateway protocol, image publication, and benchmark suites above |

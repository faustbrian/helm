...
`cargo test --quiet`: 1,556 passed, 0 failed. `just lint`, `just build`,
...
| AC-23 | Existing v7 projects have tested migration and rollback | Partial | TOML-to-YAML semantic migration; watched-root-scoped daemon IPC/CLI inventory; secret-free typed v7 config/container/image/mount/route/trust/runtime evidence; bounded generated-environment metadata, project-scoped hosts/Caddy routes, and public Caddy CA revisions; randomized user-only exact environment rollback capture and concrete managed-environment binding without project `.env` mutation; exact accepted-revision legacy-config reopening with service identity, mount, credential-field, and redacted typed credential binding; daemon-side exact binding of no-op and normally reconciled workload, stateless logical-resource, and ephemeral targets; daemon-side exact composition of all seven logical provider families against one prepared instance, tenant plan, active logical record, credential, and ownership-reconstructed target container with ambiguity refusal; shared recoverable-provider lifecycle plus exact accepted-container and configured/observed-volume strategy binding with recovery-first target restore, rollback source retention, and confirmation-only retirement; exact accepted driver, project, service, kind, container-label, immutable Engine ID, logical-data identity, and configured/observed named-volume binding for PostgreSQL, MySQL-family, MongoDB, SQL Server, Redis, Valkey, MinIO, and RabbitMQ sources; separate typed v7 Engine command targets with per-command exact legacy-label revalidation instead of false v8 ownership and one bounded attached/streaming transport shared with v8-owned sessions; tested PostgreSQL v7 custom-dump streaming into accepted-revision-bound private recovery, pre-mutation tamper refusal, replay-safe deterministic v8 target reset/restore, target catalog verification, retained-source verification, and confirmation-time exact Engine container/volume retirement with stopped-container use scans and ambiguity refusal; tested MySQL/MariaDB accepted-image flavor binding, schema-neutral streaming dump, accepted-revision-bound private recovery, pre-mutation tamper refusal, replay-safe deterministic v8 schema reset/restore, source and target identity verification, and shared exact Engine retirement; tested MongoDB database-scoped archive streaming, accepted-revision-bound private recovery, pre-mutation tamper refusal, explicit source-to-target namespace mapping, replay-safe deterministic v8 database/user rebuild, source and target identity verification, and shared exact Engine retirement; tested SQL Server checksummed native backup streaming, accepted-revision-bound private recovery, pre-mutation tamper refusal, replay-safe deterministic v8 database/login rebuild, source and target identity verification, and shared exact Engine retirement; tested Redis/Valkey binary-safe keyspace and TTL streaming, deterministic raw-key namespacing, accepted-revision-bound private recovery, pre-mutation tamper refusal, staged exact-prefix replacement, source and target identity verification, and shared exact Engine retirement; tested MinIO unversioned current-object streaming, explicit version-history refusal, accepted-revision-bound private recovery, pre-mutation tamper refusal, replay-safe exact-bucket replacement, source and target identity verification, and shared exact Engine retirement; tested RabbitMQ empty-message topology export, exact salted credential and default-vhost verification, cross-scope and queued-message refusal, accepted-revision-bound private recovery, deterministic vhost and user remapping, pre-mutation tamper refusal, replay-safe exact-vhost replacement, source and target verification, and shared exact Engine retirement; private identity-bound full-gateway snapshot backup, verified replacement, tamper-blocked rollback, and target revision proof; private identity-bound legacy CA backup, exact Stackctl CA cutover, tamper-blocked legacy trust restoration without original files, and project-safe shared-trust retention; exact legacy-label and volume-drift blockers; fresh-scan confirmation tokens and append-only SQLite acceptance with stale-evidence and identity-collision refusal; daemon acceptance now durably binds the deterministic project-wide service, volume, route, trust, and environment checkpoint plan and exact replay before success; evidence-bound deployment plus service/volume/route/trust/environment adapter selection for every v7 driver; schema-18 immutable project-wide adapter journal with a verified preparation barrier and restart-safe monotonic transitions; exact execution-scoped per-resource strategy registry that can borrow typed providers and a recovery-first resumable preparation coordinator; project-wide replay-safe cutover with atomic route/application/environment desired state, reverse-order rollback with atomic restored state and retained targets, and confirmation-gated source retirement; reversible PostgreSQL, MySQL/MariaDB, MongoDB, SQL Server, Redis, Valkey, MinIO, and empty-message RabbitMQ provider tests | Wire the composed logical-data, named-volume, gateway, trust, and environment adapters plus the project-wide preparation/cutover/rollback coordinator into daemon workers; add queued-message compatibility; prove live rollback |
...
| AC-28 | Relevant unit, integration, migration, chaos, platform, build, and lint checks pass | Partial | 1,556 local tests plus lint/build at this snapshot; Unix architecture CI definition | Required live platform, migration breadth, gateway protocol, image publication, and benchmark suites above |

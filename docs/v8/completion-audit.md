...
`cargo test --quiet`: 1,433 passed, 0 failed. `just lint`, `just build`,
...
| AC-18 | Persistent resources are never pruned implicitly | Implemented | retention policy tests; orphan stop behavior; exact seven-day disposable GC tests; common token-bound PostgreSQL, MySQL/MariaDB, MongoDB, and SQL Server prune execution; keep-data-default uninstall and pre-mutation delete-data refusal | Complete recovery/deletion coverage and uninstall delete-data execution remain incomplete |
...
| AC-22 | Removing/restoring config follows retention rules | Partial | atomic orphaning, credential disablement, adoption, PostgreSQL, MySQL/MariaDB, MongoDB, and SQL Server backup, crash-replayable PostgreSQL, MySQL/MariaDB, MongoDB, and SQL Server prune, isolated SQL Server target convergence with verified native restore and tenant evidence, disposable GC, reversible PostgreSQL, MySQL/MariaDB, and MongoDB restore/cutover/confirm/rollback, and explicit safe uninstall-mode tests | Remaining persistent-service adapters, SQL Server cutover/rollback, delete-data uninstall execution, and live rename/remove/restore acceptance |
| AC-23 | Existing v7 projects have tested migration and rollback | Partial | TOML-to-YAML semantic migration; reversible PostgreSQL, MySQL/MariaDB, and MongoDB backup/restore/cutover/confirm/rollback tests; verified SQL Server native backup, isolated restore, and tenant evidence | Complete v7 inventory and adapters for remaining data services, routes, trust, app runtimes, volumes, and generated environment |
...
| AC-28 | Relevant unit, integration, migration, chaos, platform, build, and lint checks pass | Partial | 1,433 local tests plus lint/build at this snapshot; Unix architecture CI definition | Required live platform, migration breadth, gateway protocol, image publication, and benchmark suites above |

...
`cargo test --quiet`: 807 passed, 0 failed, and 1 CI-owned live Engine test
...
| AC-28 | Relevant unit, integration, recovery, chaos, platform, build, and lint checks pass | Partial | 807 local tests plus one explicit CI-owned live Engine acceptance test, format, all-target Clippy policy, build, host-dependency audit, immutable-workflow-action audit, guarded durable cancellation of queued daemon operations, bounded independent workload and compatibility-instance reconciliation, pre-shared container observation reuse for interrupted ephemeral cleanup, pass-wide post-shared container observation reuse across orphan access revocation, cleanup, applications, workers, dedicated services, and transient provisioning jobs, bounded dedicated project-service convergence after volume eligibility, pass-wide volume observation reuse across shared and dedicated retained volumes, complete retained-volume ownership preflight before bounded independent mutation, pass-scoped shared network observations, and equal-runtime materialization deduplication at this snapshot; release-build and full-suite Unix architecture CI definition | CI artifacts and external platform, recovery, image publication, and benchmark records assigned by `external-verification.md` |

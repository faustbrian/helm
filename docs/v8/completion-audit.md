...
`cargo test --quiet`: 704 passed, 0 failed. `just lint` (format plus all-target
...
|---|---|---|---|---|
| AC-01 | One login-started per-user daemon is authoritative | Partial | `src/control_plane/daemon/unix_daemon_runtime.rs`; one-command rollback-aware setup, singleton lease, transactional login-service installation, manager-backed status, and Unix service-manager tests | Live macOS and Linux login/reboot records |
...
| AC-28 | Relevant unit, integration, recovery, chaos, platform, build, and lint checks pass | Partial | 704 local tests plus format, all-target Clippy policy, build, and host audit at this snapshot; Unix architecture CI definition | Required live platform, recovery, gateway protocol, image publication, and benchmark suites above |

...
| AC-28 | Relevant unit, integration, recovery, chaos, platform, build, and lint checks pass | Partial | 798 local tests plus format, all-target Clippy policy, build, host-dependency audit, immutable-workflow-action audit, bounded independent reconciliation, pass-wide workload observation reuse, and equal-runtime materialization deduplication at this snapshot; Unix architecture CI definition | CI artifacts and external platform, recovery, image publication, and benchmark records assigned by `external-verification.md` |

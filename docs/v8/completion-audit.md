...
`cargo test --quiet`: 1,509 passed, 0 failed. `just lint`, `just build`,
...
| AC-23 | Existing v7 projects have tested migration and rollback | Partial | TOML-to-YAML semantic migration; watched-root-scoped daemon IPC/CLI inventory; secret-free typed v7 config/container/image/mount/route/trust/runtime evidence; bounded generated-environment metadata, project-scoped hosts/Caddy routes, and public Caddy CA revisions; randomized user-only exact environment rollback capture and verification; exact legacy-label and volume-drift blockers; fresh-scan confirmation tokens and append-only SQLite acceptance with stale-evidence and identity-collision refusal; evidence-bound deployment plus service/volume/route/trust/environment adapter selection for every v7 driver; reversible PostgreSQL, MySQL/MariaDB, MongoDB, and SQL Server backup/restore/cutover/confirm/rollback tests | Execute the selected app/route/trust/volume adapters, add remaining data-service operations, and prove live rollback |
...
| AC-28 | Relevant unit, integration, migration, chaos, platform, build, and lint checks pass | Partial | 1,509 local tests plus lint/build at this snapshot; Unix architecture CI definition | Required live platform, migration breadth, gateway protocol, image publication, and benchmark suites above |

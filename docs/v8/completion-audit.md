...
`cargo test --quiet`: 1,431 passed, 0 failed. `just lint`, `just build`,
...
| AC-22 | Removing/restoring config follows retention rules | Partial | atomic orphaning, credential disablement, adoption, PostgreSQL, MySQL/MariaDB, MongoDB, and SQL Server backup, crash-replayable PostgreSQL, MySQL/MariaDB, MongoDB, and SQL Server prune, disposable GC, reversible PostgreSQL, MySQL/MariaDB, and MongoDB restore/cutover/confirm/rollback, and explicit safe uninstall-mode tests | Remaining persistent-service adapters, SQL Server restore, delete-data uninstall execution, and live rename/remove/restore acceptance |
| AC-23 | Existing v7 projects have tested migration and rollback | Partial | TOML-to-YAML semantic migration; reversible PostgreSQL, MySQL/MariaDB, and MongoDB backup/restore/cutover/confirm/rollback tests; verified native SQL Server backup | Complete v7 inventory and adapters for remaining data services, routes, trust, app runtimes, volumes, and generated environment |
...
| AC-28 | Relevant unit, integration, migration, chaos, platform, build, and lint checks pass | Partial | 1,431 local tests plus lint/build at this snapshot; Unix architecture CI definition | Required live platform, migration breadth, gateway protocol, image publication, and benchmark suites above |

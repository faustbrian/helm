...
`cargo test --quiet`: 721 passed, 0 failed. `just lint` (format plus all-target
...
|---|---|---|---|---|
| AC-01 | One login-started per-user daemon is authoritative | Partial | `src/control_plane/daemon/unix_daemon_runtime.rs`; one-command rollback-aware setup, canonical watched-root validation, singleton lease, IPC-readiness-gated transactional login-service installation, verified manager cleanup, manager-and-IPC-backed status, bounded host logging, and Unix service-manager tests | Live macOS and Linux login/reboot records |

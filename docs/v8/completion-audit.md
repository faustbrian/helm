...
`cargo test --quiet`: 1,402 passed, 0 failed. `just lint`, `just build`,
...
| AC-18 | Persistent resources are never pruned implicitly | Implemented | retention policy tests; orphan stop behavior; exact seven-day disposable GC tests; explicit token-bound PostgreSQL prune execution | Complete multi-service deletion and uninstall delete-data workflows remain incomplete |
...
| AC-22 | Removing/restoring config follows retention rules | Partial | atomic orphaning, credential disablement, adoption, backup, restore, disposable GC, and crash-replayable PostgreSQL prune tests | Prune adapters for other persistent services, complete uninstall modes, and live rename/remove/restore acceptance |

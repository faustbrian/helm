...
| AC-16 | Logical resources and credentials converge idempotently | Partial | PostgreSQL, MySQL/MariaDB, MongoDB, Redis/Valkey, object-store, RabbitMQ, Mailpit, SQL Server, dedicated Dragonfly, dedicated Garage, dedicated LocalStack, dedicated Memcached, dedicated Elasticsearch, dedicated OpenSearch, dedicated Meilisearch, dedicated RustFS root, dedicated Typesense, and dedicated Soketi preparation/reconciliation tests; real-directory-locked shared and project configuration and credential publication | RustFS bucket provisioning through a pinned external client; live authenticated readiness and drift records per advertised service |

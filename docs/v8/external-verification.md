...
| `CI` | Isolated-home real Laravel bootstrap failure rejection, framework `/up` health, strict CA-file HTTPS verification, application command, worker liveness, scheduler execution, source visibility, daemon restart, stable CA identity, and exact owned-resource cleanup on native Linux x86_64 and arm64 | `laravel-acceptance-linux-*` |

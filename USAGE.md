...
- `--port-seed <SEED>`
- `--persist-ports` (requires `--random-ports`)
- `--write-env`
- `--with-project-deps`
- `--with-data`
- `--parallel <N>` (default: `1`)
...
- `--kind <KIND>`
- `--with-project-deps`
- `--force-project-dep-down` (requires `--with-project-deps`)
- `--parallel <N>` (default: `1`)
...
Open an interactive connection/session to a service.
...
- `--tty`
- `--no-tty`
- Trailing command is required after flags.
...
- `--port-seed <SEED>`
- `--write-env`
- Trailing command is required (examples: `up`, `down`, `status --format json`).

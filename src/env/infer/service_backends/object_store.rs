//! Object-store backend env inference (S3-compatible providers).
...
pub(super) fn apply(vars: &mut HashMap<String, String>, service: &ServiceConfig) {
    ...
    insert_if_absent(vars, "AWS_URL", object_store_public_url(service));
    insert_if_absent(vars, "AWS_USE_PATH_STYLE_ENDPOINT", "true".to_owned());
}

fn object_store_public_url(service: &ServiceConfig) -> String {
    service
        .resolved_domain_urls()
        .into_iter()
        .next()
        .unwrap_or_else(|| service_endpoint(service))
}

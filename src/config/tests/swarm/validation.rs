...
fn load_config_with_rejects_duplicate_swarm_target_names() {
    ...
    let error = load_config_with(Some(&config_path), None).expect_err("expected duplicate error");
...
fn load_config_with_rejects_unknown_swarm_dependency() {
    ...
    let error = load_config_with(Some(&config_path), None).expect_err("expected dep error");
    assert!(
        error
            .to_string()
...
fn load_config_with_rejects_empty_swarm_git_repo() {
    ...
    let error = load_config_with(Some(&config_path), None).expect_err("expected git repo error");

...
#[test]
fn apply_runtime_env_testing_namespace_keeps_base_ports() -> Result<()> {
    let mut config = Config {
        schema_version: 1,
        container_prefix: Some("test".to_owned()),
    ...
    apply_runtime_env(&mut config, "testing-2f1a9c4d")?;

    assert_eq!(
        config.service[0].container_name()?,
        "test-db-testing-2f1a9c4d"
    );
    assert_eq!(config.service[0].port, 54_320);
    Ok(())
}
...
fn default_env_file_name_maps_test_to_testing_file() -> Result<()> {
    ...
    assert_eq!(
        default_env_file_name(Some("testing-2f1a9c4d"))?,
        ".env.testing"
    );
...
fn resolved_domains_prefers_primary_then_unique_aliases() {
    ...
    assert_eq!(
        app.resolved_domain_urls(),
        vec![
            "https://main.site".to_owned(),
...
#[test]
fn resolved_domain_urls_respect_scheme_and_keep_prefixed_values() {
    let mut app = mysql_service("app");
    app.kind = Kind::App;
    app.driver = Driver::Frankenphp;
    app.scheme = Some("http".to_owned());
    app.domain = Some("main.site".to_owned());
    app.domains = Some(vec!["https://other.site".to_owned()]);

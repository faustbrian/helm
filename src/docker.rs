...
#[cfg(test)]
static TEST_DOCKER_COMMAND: std::sync::OnceLock<std::sync::Mutex<Option<String>>> =
    std::sync::OnceLock::new();
#[cfg(test)]
static TEST_DOCKER_SCOPE_LOCK: std::sync::OnceLock<std::sync::Mutex<()>> =
    std::sync::OnceLock::new();
#[cfg(test)]
static TEST_DRY_RUN: std::sync::OnceLock<std::sync::Mutex<()>> = std::sync::OnceLock::new();
    ...
    F: FnOnce() -> T,
{
    ...
    let lock = TEST_DOCKER_COMMAND.get_or_init(Default::default);
    let mut lock = match lock.lock() {
        Ok(lock) => lock,
        Err(err) => err.into_inner(),
    };
    let previous = lock.clone();
    *lock = Some(command.to_owned());
    drop(lock);
    ...
    let mut lock = match TEST_DOCKER_COMMAND.get_or_init(Default::default).lock() {
        Ok(lock) => lock,
        Err(err) => err.into_inner(),
    };
    *lock = previous;
...
pub(crate) fn docker_command() -> String {
    #[cfg(test)]
    {
        if let Some(lock) = TEST_DOCKER_COMMAND.get() {
            let guard = match lock.lock() {
                Ok(guard) => guard,
                Err(err) => err.into_inner(),
            };
            if let Some(command) = guard.clone() {
                return command;
            }
        }
    }
...
#[cfg(test)]
mod tests {
    use crate::docker::{is_dry_run, set_dry_run, with_dry_run_lock};

    #[test]
    fn with_dry_run_lock_restores_previous_dry_run_value_after_success() {
        let previous = is_dry_run();
        set_dry_run(previous);
        with_dry_run_lock(|| {
            assert!(is_dry_run());
        ...
        assert_eq!(is_dry_run(), previous);
    }

    #[test]
    fn with_dry_run_lock_restores_previous_dry_run_value_after_panic() {
        let previous = is_dry_run();

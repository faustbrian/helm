...
use crate::control_plane::migration::{
    ...
    SqlServerMigrationOperations, SqlServerMigrationOperationsOptions, confirm_migration,
    rollback_migration,
};
    ...
    Entropy: CredentialEntropy,
{
    match decision_resource_kind(options)?.as_str() {
        ...
        "sqlserver_database" => execute_sql_server_decision(engine, entropy, options).await,
...
async fn execute_sql_server_decision<E, Entropy>(
    engine: &mut E,
    entropy: &Entropy,
    options: &MigrationDecisionExecutionOptions,
) -> Result<crate::control_plane::migration::MigrationExecutionResult, String>
    ...
    Entropy: CredentialEntropy,
{
    ...
    let source_administrator_id = format!("shared/{fingerprint_id}/sqlserver-bootstrap");
    ...
    let source_plan = SqlServerSharedInstancePlan::new(
        &options.shared,
    ...
    .map_err(|error| error.to_string())?;
    let source_resources = plan_sql_server_project_resources(
        source.project_id(),
        source.service_id(),
        &source_plan,
        CredentialSecret::new(source_credential.secret().to_owned()),

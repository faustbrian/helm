    ...
    Store: StateStore,
{
    ...
    match request.payload() {
        ...
        IpcPayload::Reconcile => {
            if let Err(error) = event_journal.append(request.request_id(), IpcEventKind::Accepted) {
            ...
            match reconcile_watched_roots(control_plane, discovery_options, now_unix_seconds) {
                Ok(result) => {
                    if let Err(error) =
                        event_journal.append(request.request_id(), IpcEventKind::Completed)
                    {
                        return IpcResponse::failure(
                            request.request_id(),
                            vec![IpcDiagnostic::new(
                ...
                Err(error) => {
                    ...
                    if let Err(journal_error) = event_journal.append(
                        request.request_id(),
                        IpcEventKind::Failed {

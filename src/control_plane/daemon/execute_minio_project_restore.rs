...
/// Restores one unversioned bucket after durably recording its current objects.
pub(crate) async fn execute_minio_project_restore<E>(
    engine: &E,
    ...
    E: CommandExecutor + ContainerDiscovery,
{
    ...
    let mut store = SqliteStateStore::open(&options.state_database_path)
        .map_err(|error| format!("could not open MinIO restore state: {error}"))?;
    let logical = one(
        store
            .logical_resources()
    ...
    let credential = one(
        store
            ...
            .collect(),
        "active MinIO tenant credential",
    )?;
    let recovery = one(
        store
    ...
    if safety_recovery_id == recovery.recovery_point_id() {
        return Err("MinIO restore safety recovery collides with the selected point".to_owned());
    }
    ...
    if safety_matches.is_empty() {
        ...
        return Err("MinIO restore safety recovery does not match the active bucket".to_owned());
    }
    restore_minio_bucket(
        engine,
        &container,
    ...
    E: ContainerDiscovery,
{
    ...
    one(matches, "owned MinIO shared container")
}
...
fn validate_options(options: &ProjectRestoreExecutionOptions) -> Result<(), String> {
    if options.operation.kind() != "minio_bucket_policy"
        ...
        || options.shared_target()?.profile().implementation() != "minio"
        || options.shared_target()?.fingerprint().as_str()
            != options.operation.compatibility_fingerprint()
    {
        return Err("MinIO restore execution options are incomplete or incompatible".to_owned());
    }

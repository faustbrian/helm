...
/// Typed request for one deterministically named Stackctl-owned volume.
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) struct VolumeCreateOptions {
    name: String,
    ...
    pub(crate) fn new(
        ...
        if name.is_empty() {
            return Err(EngineError::InvalidRequest {
                detail: "managed volume name must not be empty".to_owned(),
            });
        }

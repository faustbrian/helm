...
"#;
    ...
    assert_eq!(
        desired_from(source)
            ...
            .to_string(),
        "service 'app' declares PHP extensions but preset '<none>' does not provide the public PHP extension installer contract"
    );
}

#[test]
fn desired_state_accepts_valid_public_installer_extension_names() {
    let source = r#"
schema_version: 8
services:
  app:
    preset: laravel
    php_extensions: [xsl, calendar, soap]
"#;

    let desired = desired_from(source).expect("public installer extensions");

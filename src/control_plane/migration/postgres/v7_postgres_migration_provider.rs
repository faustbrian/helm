use super::{
    V7PostgresMigrationProviderOptions, V7PostgresSourceRetirement, backup_v7_postgres_database,
    restore_v7_postgres_target, verify_v7_postgres_source, verify_v7_postgres_target,
};
...
impl<'operation, E, R> V7PostgresMigrationProvider<'operation, E, R>
where
    E: CommandExecutor + V7ContainerCommandExecutor + Sync,
    R: V7PostgresSourceRetirement,
{
    pub(crate) fn new(
        executor: &'operation E,
        retirement: R,
        ...
        let source_target = options
            .source
            .command_target()
            .map_err(|error| operation_error("v7 PostgreSQL command target is invalid", error))?;
    ...
    fn validate_source(
        ...
        if source != &self.source {
            return Err(MigrationOperationError::new(
                "v7 PostgreSQL operation source differs from the accepted provider identity",
            ));
        }
    ...
    fn verify_recovery(
        ...
        if checkpoint.phase() != V7MigrationAdapterCheckpointPhase::RecoveryVerified
            ...
            || checkpoint.adapter_kind() != "postgres-logical-database"
            || !checkpoint.requires_recovery()
        {
            return Err(MigrationOperationError::new(
                "v7 PostgreSQL recovery checkpoint does not match the accepted source",
            ));
        }
        let reference = checkpoint.recovery_reference().ok_or_else(|| {
            MigrationOperationError::new("v7 PostgreSQL recovery checkpoint has no reference")
        })?;
        let stored = open_stored_backup_artifact(reference)
            .map_err(|error| operation_error("open v7 PostgreSQL recovery", error))?;
        let verified =
            verify_stored_backup_artifact(&stored, self.options.verified_at_unix_seconds)
                .map_err(|error| operation_error("verify v7 PostgreSQL recovery", error))?;
        ...
        {
            return Err(MigrationOperationError::new(
                "v7 PostgreSQL recovery artifact differs from its durable checkpoint",
            ));
        }
...
impl<E, R> V7RecoverableMigrationProvider<V7LogicalDataMigrationSource>
    for V7PostgresMigrationProvider<'_, E, R>
where
    E: CommandExecutor + V7ContainerCommandExecutor + Sync,
    R: V7PostgresSourceRetirement,
{
    ...
    fn verify_target<'operation>(
        ...
        Box::pin(async move {
            ...
            if target_reference != self.target_reference {
                return Err(MigrationOperationError::new(
                    "v8 PostgreSQL target reference differs from the prepared resource",
                ));
            }
            verify_v7_postgres_target(
                self.executor,
...
fn validate_options(
    options: &V7PostgresMigrationProviderOptions<'_>,
) -> Result<(), MigrationOperationError> {
    ...
    .map_err(MigrationOperationError::new)?;
    let source_database = source_database_name(options.source)?;
    let target = options.target_logical_resource;
    let invalid = options.source.driver() != "postgres"
    ...
    if invalid {
        return Err(MigrationOperationError::new(
            "v7 PostgreSQL provider does not describe one accepted source and owned v8 target",
        ));
    }
...
fn source_database_name(
    ...
    if source.logical_data().len() != 1 {
        return Err(MigrationOperationError::new(
            "v7 PostgreSQL source requires exactly one accepted database identity",
        ));
    }
    source
        .logical_data()
        ...
        .map(String::as_str)
        .ok_or_else(|| {
            MigrationOperationError::new("v7 PostgreSQL source has no accepted database identity")
        })
}

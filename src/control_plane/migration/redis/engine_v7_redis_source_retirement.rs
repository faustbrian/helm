use super::V7RedisSourceRetirement;
...
impl<E> V7RedisSourceRetirement for EngineV7RedisSourceRetirement<'_, E>
where
        ...
        Box::pin(async move {
            let container = source.command_target().map_err(|error| {
                MigrationOperationError::new(format!(
                    "v7 Redis-compatible retirement target is invalid: {error}"
                ))
            })?;
            let target =
                V7ContainerRetirementTarget::new(container, source.named_volumes().to_vec())
                    .map_err(|error| {
                        MigrationOperationError::new(format!(
                            "v7 Redis-compatible retirement volumes are invalid: {error}"
                        ))
                    })?;
            self.engine
                ...
                .map_err(|error| {
                    MigrationOperationError::new(format!(
                        "retire accepted v7 Redis-compatible source: {error}"
                    ))
                })
        })
    }

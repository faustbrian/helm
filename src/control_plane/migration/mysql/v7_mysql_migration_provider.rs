use super::{
    V7MySqlMigrationProviderOptions, V7MySqlSourceRetirement, accepted_v7_mysql_flavor,
    backup_v7_mysql_database, restore_v7_mysql_target, verify_v7_mysql_source,
    verify_v7_mysql_target,
};
use crate::control_plane::engine::{
    CommandExecutor, V7ContainerCommandExecutor, V7ContainerCommandTarget,
};
...
impl<'operation, E, R> V7MySqlMigrationProvider<'operation, E, R>
where
    E: CommandExecutor + V7ContainerCommandExecutor + Sync,
    R: V7MySqlSourceRetirement,
{
    pub(crate) fn new(
        executor: &'operation E,
        retirement: &'operation mut R,
        ...
        let source_target = options
            .source
            .command_target()
            .map_err(|error| operation_error("v7 MySQL-family command target is invalid", error))?;
        ...
        let target_reference = format!(
            "{}:{}:{}",
            flavor_label(options.flavor),
            options.target_logical_resource.logical_resource_id(),
            options.target_plan.schema_name()
        );
    ...
    fn validate_source(
        ...
        if source != &self.source {
            return Err(MigrationOperationError::new(
                "v7 MySQL-family operation source differs from accepted provider identity",
            ));
        }
        Ok(())
    }
    ...
    fn verify_recovery(
        ...
        if checkpoint.phase() != V7MigrationAdapterCheckpointPhase::RecoveryVerified
            ...
            || checkpoint.adapter_kind() != "mysql-logical-database"
            || !checkpoint.requires_recovery()
        {
            return Err(MigrationOperationError::new(
                "v7 MySQL-family recovery checkpoint does not match the accepted source",
            ));
        }
        let reference = checkpoint.recovery_reference().ok_or_else(|| {
            MigrationOperationError::new("v7 MySQL-family recovery checkpoint has no reference")
        })?;
        let stored = open_stored_backup_artifact(reference)
            .map_err(|error| operation_error("open v7 MySQL-family recovery", error))?;
        let verified =
            verify_stored_backup_artifact(&stored, self.options.verified_at_unix_seconds)
                .map_err(|error| operation_error("verify v7 MySQL-family recovery", error))?;
        ...
        {
            return Err(MigrationOperationError::new(
                "v7 MySQL-family recovery differs from its durable checkpoint",
            ));
        }
        Ok(stored)
    }
...
impl<E, R> V7RecoverableMigrationProvider<V7LogicalDataMigrationSource>
    for V7MySqlMigrationProvider<'_, E, R>
where
    E: CommandExecutor + V7ContainerCommandExecutor + Sync,
    R: V7MySqlSourceRetirement,
{
    ...
    fn verify_target<'operation>(
        ...
        Box::pin(async move {
            ...
            if target_reference != self.target_reference {
                return Err(MigrationOperationError::new(
                    "v8 MySQL-family target reference differs from prepared resource",
                ));
            }
            verify_v7_mysql_target(
                self.executor,
...
fn validate_options(
    options: &V7MySqlMigrationProviderOptions<'_>,
) -> Result<(), MigrationOperationError> {
    ...
    .map_err(MigrationOperationError::new)?;
    if accepted_v7_mysql_flavor(options.accepted, options.source)
        .map_err(MigrationOperationError::new)?
        != options.flavor
    {
        return Err(MigrationOperationError::new(
            "v7 MySQL-family flavor differs from accepted image evidence",
        ));
    }
    ...
    let expected_admin_service = flavor_label(options.flavor);
    let target = options.target_logical_resource;
    let invalid = options.source.driver() != "mysql"
        ...
        || options.target_plan.flavor() != options.flavor
        ...
        || options.administrator.service_id() != expected_admin_service
        || options.administrator.username() != "root"
    ...
    if invalid {
        return Err(MigrationOperationError::new(
            "v7 MySQL-family provider does not describe one accepted source and owned v8 target",
        ));
    }
    Ok(())
}
...
fn source_database_name(
    ...
    if source.logical_data().len() != 1 {
        return Err(MigrationOperationError::new(
            "v7 MySQL-family source requires exactly one accepted database identity",
        ));
    }
    source
        .logical_data()
        ...
        .map(String::as_str)
        .ok_or_else(|| {
            MigrationOperationError::new("v7 MySQL-family source has no accepted database identity")
        })
}

const fn flavor_label(flavor: MySqlFlavor) -> &'static str {
    match flavor {

use super::V7MinioSourceRetirement;
...
/// Engine-backed confirmation cleanup for one accepted v7 MinIO source.
pub(crate) struct EngineV7MinioSourceRetirement<'operation, E> {
    engine: &'operation mut E,
}

impl<'operation, E> EngineV7MinioSourceRetirement<'operation, E> {
    pub(crate) const fn new(engine: &'operation mut E) -> Self {
        Self { engine }
    }
}

impl<E> V7MinioSourceRetirement for EngineV7MinioSourceRetirement<'_, E>
where
        ...
        Box::pin(async move {
            let container = source.command_target().map_err(|error| {
                MigrationOperationError::new(format!(
                    "v7 MinIO retirement target is invalid: {error}"
                ))
            })?;
            let target =
                V7ContainerRetirementTarget::new(container, source.named_volumes().to_vec())
                    .map_err(|error| {
                        MigrationOperationError::new(format!(
                            "v7 MinIO retirement volumes are invalid: {error}"
                        ))
                    })?;
            self.engine
                ...
                .map_err(|error| {
                    MigrationOperationError::new(format!(
                        "retire accepted v7 MinIO source: {error}"
                    ))
                })
        })
    }

...
pub(super) fn mongodb_connection_uri(username: &str, password: &str, database: &str) -> String {
    let username = percent_encode(username);
    let password = percent_encode(password);
    let database = percent_encode(database);

    format!("mongodb://{username}:{password}@127.0.0.1:27017/{database}?authSource={database}")
}
...
mod tests {
    ...
    fn encodes_every_reserved_and_non_ascii_credential_byte() {
        assert_eq!(
            mongodb_connection_uri("user name", "päss:/?#[]@", "stackctl-db"),

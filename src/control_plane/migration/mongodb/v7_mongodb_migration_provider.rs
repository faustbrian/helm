use super::{
    V7MongoDbMigrationProviderOptions, V7MongoDbSourceRetirement, backup_v7_mongodb_database,
    restore_v7_mongodb_target, verify_v7_mongodb_source, verify_v7_mongodb_target,
};
...
impl<'operation, E, R> V7MongoDbMigrationProvider<'operation, E, R>
where
    E: CommandExecutor + V7ContainerCommandExecutor + Sync,
    R: V7MongoDbSourceRetirement,
{
    pub(crate) fn new(
        executor: &'operation E,
        retirement: R,
        ...
        let source_target = options
            .source
            .command_target()
            .map_err(|error| operation_error("v7 MongoDB command target is invalid", error))?;
    ...
    fn validate_source(
        ...
        if source != &self.source {
            return Err(MigrationOperationError::new(
                "v7 MongoDB operation source differs from accepted provider identity",
            ));
        }
        Ok(())
    }
    ...
    fn verify_recovery(
        ...
        if checkpoint.phase() != V7MigrationAdapterCheckpointPhase::RecoveryVerified
            ...
            || checkpoint.adapter_kind() != "mongodb-logical-database"
            || !checkpoint.requires_recovery()
        {
            return Err(MigrationOperationError::new(
                "v7 MongoDB recovery checkpoint does not match the accepted source",
            ));
        }
        let reference = checkpoint.recovery_reference().ok_or_else(|| {
            MigrationOperationError::new("v7 MongoDB recovery checkpoint has no reference")
        })?;
        let stored = open_stored_backup_artifact(reference)
            .map_err(|error| operation_error("open v7 MongoDB recovery", error))?;
        let verified =
            verify_stored_backup_artifact(&stored, self.options.verified_at_unix_seconds)
                .map_err(|error| operation_error("verify v7 MongoDB recovery", error))?;
        ...
        {
            return Err(MigrationOperationError::new(
                "v7 MongoDB recovery differs from its durable checkpoint",
            ));
        }
        Ok(stored)
    }
...
impl<E, R> V7RecoverableMigrationProvider<V7LogicalDataMigrationSource>
    for V7MongoDbMigrationProvider<'_, E, R>
where
    E: CommandExecutor + V7ContainerCommandExecutor + Sync,
    R: V7MongoDbSourceRetirement,
{
    ...
    fn verify_target<'operation>(
        ...
        Box::pin(async move {
            ...
            if target_reference != self.target_reference {
                return Err(MigrationOperationError::new(
                    "v8 MongoDB target reference differs from prepared resource",
                ));
            }
            verify_v7_mongodb_target(
                self.executor,
...
fn validate_options(
    options: &V7MongoDbMigrationProviderOptions<'_>,
) -> Result<(), MigrationOperationError> {
    ...
    let target_metadata = options.target_container.metadata();
    let expected_administrator_id = format!(
        "migration/{}/mongodb-bootstrap",
        target_metadata.resource_id().unwrap_or_default()
    );
    let invalid = options.source.driver() != "mongodb"
        ...
        || !options
            .target_plan
            .matches_credential(options.target_credential, options.administrator.secret())
        || options.target_credential.lifecycle() != CredentialLifecycle::Active
        || options.administrator.credential_id() != expected_administrator_id
        ...
        || target_metadata.installation_id() != options.installation_id
        || target_metadata.kind() != ResourceKind::ProjectService
        || target_metadata.project_id() != Some(target.project_id())
        || target_metadata.resource_id().is_none()
        || target_metadata.retention() != RetentionClass::Persistent
        || target_metadata.compatibility_fingerprint() != target.compatibility_fingerprint();
    if invalid {
        return Err(MigrationOperationError::new(
            "v7 MongoDB provider does not describe one accepted source and owned v8 target",
        ));
    }
    Ok(())
}
...
fn source_database_name(
    ...
    if source.logical_data().len() != 1 {
        return Err(MigrationOperationError::new(
            "v7 MongoDB source requires exactly one accepted database identity",
        ));
    }
    source
        .logical_data()
        ...
        .map(String::as_str)
        .ok_or_else(|| {
            MigrationOperationError::new("v7 MongoDB source has no accepted database identity")
        })
}

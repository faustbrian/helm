...
pub(crate) fn handle_artisan(
    ...
    if command.is_empty() {
        anyhow::bail!(
            "No artisan command specified. Usage: helm artisan [--target <name>] -- <command>"
        );
    }
    ...
    let serve_target = config::resolve_app_service(&effective_config, target)?;
    ...
    if is_test_command {
        ensure_test_services_running(&effective_config, &workspace_root, &inferred_env)?;
    }

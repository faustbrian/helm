...
use super::{
    ...
    ensure_artisan_ansi_flag, handle_artisan, is_artisan_test_command,
    load_artisan_test_base_config, remove_artisan_env_overrides, resolve_artisan_tty,
};
use crate::config;
...
#[test]
fn handle_artisan_rejects_library_project_type() {
    ...
    let result = handle_artisan(
        &config,
        HandleArtisanOptions {
            service: None,
            kind: None,
            profile: None,
    ...
    assert!(
        result
            .expect_err("library mode should reject artisan")
            .to_string()

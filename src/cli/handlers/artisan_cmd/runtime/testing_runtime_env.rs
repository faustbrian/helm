...
fn adaptive_pool_size_from_signals(
    ...
    let cpu_bound = if effective_cpus >= 12 {
        4
    } else if effective_cpus >= 6 {
        3
    } else if effective_cpus >= 2 {
        2
    } else {
        1
    };
    ...
    cpu_bound.min(mem_bound).clamp(1, 4)
}
...
mod tests {
    ...
    use super::{
        ...
        resolve_pooled_runtime_env_name, testing_runtime_env_name,

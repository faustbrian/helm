...
/// Handles the `random ports up` CLI command.
...
pub(super) fn run_random_ports_up(
    ...
    if parallel <= 1 {
        for (runtime, uses_random_port) in &planned {
        ...
        let (app_planned, non_app_planned): (Vec<_>, Vec<_>) = planned
            .iter()
            .partition(|(runtime, _)| runtime.kind == config::Kind::App);
        ...
        rayon::ThreadPoolBuilder::new()
            ...
            .install(|| {
                non_app_planned
                    .par_iter()
                    .try_for_each(|(runtime, uses_random_port)| {
                        start_runtime_service(
                            runtime,
                            *uses_random_port,
                            quiet,
        ...
        for (runtime, uses_random_port) in app_planned {
            start_runtime_service(
                runtime,
                *uses_random_port,
                quiet,
...
fn start_runtime_service(
    ...
    if runtime.kind == config::Kind::App {
        ...
        env::merge_with_protected_https_app_urls(&mut injected_env, project_dependency_env);
        serve::run(
            runtime,
            recreate,
...
/// Returns whether the candidate port is currently available.

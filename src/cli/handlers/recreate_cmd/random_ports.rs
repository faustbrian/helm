...
pub(super) fn handle_random_ports_recreate(
    ...
    for mut runtime in selected {
        runtime.port = random_unused_port(&runtime.host, &used_ports)?;
        used_ports.insert(runtime.port);
        if runtime.driver == config::Driver::Mailhog && runtime.smtp_port.is_some() {
            let smtp_port = random_unused_port(&runtime.host, &used_ports)?;
...
fn random_unused_port(host: &str, used_ports: &HashSet<u16>) -> Result<u16> {
    for _ in 0..100 {
